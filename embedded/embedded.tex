...
\begin{frame}
  ...
  \begin{itemize}
    \item{Installing rustup target e.g. \inlinecode{thumbv7em-none-eabihf} for STM32 F4 boards}
...
\begin{frame}
  \frametitle{Blinky}
  The \inlinecode{stm32f4xx-hal} library/crate is the go to HAL crate for STM32 F4 series MCU.
...
\begin{frame}
  ...
  \lstinputlisting[linerange={10-11,30}]{src/bin/blinky.rs}

  Before we can setup any peripherals, we need to take the ownership of the peripherals.

  \lstinputlisting[linerange={12}]{src/bin/blinky.rs}

  Once the peripherals is setup a \inlinecode{loop} is used to execute code infinitely.
\end{frame}

\begin{frame}[allowframebreaks]
  \frametitle{Digital Pins}
  A digital output pin can be used to output high and low on a GPIO pin. A digital pin must be setup as an output before it can be used. In \inlinecode{stm32f4xx-hal} a digital output can be setup by doing:
  \lstinputlisting[linerange={14-15}]{src/bin/blinky.rs}

  Eventhough the setup procdure might differ, most crates should have simillar/same functions to manipulate it.
  \lstinputlisting[linerange={22}, caption={Set Digital Output to High}]{src/bin/blinky.rs}
  \lstinputlisting[linerange={24}, caption={Set Digital Output to High}]{src/bin/blinky.rs}

  A digital input is used to read digital signal. Setting up pins as a digital input is simillar to digital output:
  \lstinputlisting[linerange={15-16}]{src/bin/digital-input.rs}

  Two main methods can be used to read the digital input:
  \lstinputlisting[linerange={22}, caption={Returns True if input is high}]{src/bin/digital-input.rs}
  \lstinputlisting[linerange={23}, caption={Returns True if input is low}]{src/bin/digital-input.rs}
\end{frame}

\begin{frame}
  \frametitle{Delay}
  A delay can be used to block program execution for a specific amount of time. To setup a delay:
  \lstinputlisting[linerange={17-19}]{src/bin/blinky.rs}

  There are two different precision delays:
  \lstinputlisting[linerange={23}, caption={Delay With Millisecond Precision}]{src/bin/blinky.rs}
  \lstinputlisting[linerange={25}, caption={Delay With Microsecond Precision}]{src/bin/blinky.rs}
\end{frame}

\begin{frame}[allowframebreaks]
  \frametitle{Analog Input}
  An ADC can be used to read analog input. An ADC can be setup by doing:
  \lstinputlisting[linerange={25-29}]{src/bin/adc.rs}
  ...
  \lstinputlisting[linerange={36}]{src/bin/adc.rs}

  \pagebreak

  Before a voltage value can be read the GPIO pin must be setup to used as analog input.
  \lstinputlisting[linerange={21-22}]{src/bin/adc.rs}

  A single sample can be taken by using convert method:
  \lstinputlisting[linerange={33}]{src/bin/adc.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={17-20}]{src/bin/usart.rs}

  A single ``word''/character can be read using the \inlinecode{read} method.
  \lstinputlisting[linerange={23}]{src/bin/usart.rs}

  To send data, the \inlinecode{write} method is used instead.
  \lstinputlisting[linerange={27}]{src/bin/usart.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={31,32}]{src/bin/rng.rs}

  To generate a random number, the gen method can be used with a type anotated variable:
  \lstinputlisting[linerange={35}]{src/bin/rng.rs}
\end{frame}
...
Often times programmers want the program to panic when an error occured. This can be done using the \inlinecode{unwrap} and \inlinecode{expect} method. Both methods causes the program to panic, but in \inlinecode{expect} a custom error message is specified as a parameter.
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,303-306,312}]{src/game.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,268-270,284,312}]{src/game.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,86-94,312}]{src/game.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange={71}]{src/main.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange=43-47]{src/main.rs}
  ...
  \lstinputlisting[linerange=88-94, caption={Function that takes ownership.}]{src/lib.rs}
  \lstinputlisting[linerange=48-50, caption={Functions taking ownership usage.}]{src/main.rs}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange=129-132, caption={Methods taking reference to the object.}]{src/lib.rs}

  \pagebreak
  A mutable borrow is also allowed. Instead of \inlinecode{\&} \inlinecode{\&mut} is used instead.
  \lstinputlisting[linerange=77-86]{src/lib.rs}
\end{frame}
