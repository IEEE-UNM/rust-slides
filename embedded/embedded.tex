...
\begin{frame}
  \frametitle{Blinky}
  The \inlinecode{stm32l4xx-hal} library/crate is the go to HAL crate for STM32 L4 series MCU.
...
\begin{frame}[allowframebreaks]
  ...
  A digital output pin can be used to output high and low on a GPIO pin. A digital pin must be setup as an output before it can be used. In \inlinecode{stm32l4xx-hal} a digital output can be setup by doing:
  ...
  \lstinputlisting[linerange={18-21}]{scrolling-game/bin/digital-input.rs}

  \pagebreak

  Two main methods can be used to read the digital input:
  \lstinputlisting[linerange={27}, caption={Returns True if input is high}]{scrolling-game/bin/digital-input.rs}
  \lstinputlisting[linerange={28}, caption={Returns True if input is low}]{scrolling-game/bin/digital-input.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange={25-28}]{scrolling-game/bin/adc.rs}
  ...
  \lstinputlisting[linerange={29,35}]{scrolling-game/bin/adc.rs}
  ...
  \lstinputlisting[linerange={32}]{scrolling-game/bin/adc.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange={18-24},caption={Setting up required peripherals.}]{scrolling-game/bin/usart.rs}
  \lstinputlisting[linerange={25-31},caption={Creating Serial Port.}]{scrolling-game/bin/usart.rs}

  A single ``word''/character can be read using the \inlinecode{read} method.
  \lstinputlisting[linerange={34}]{scrolling-game/bin/usart.rs}

  To send data, the \inlinecode{write} method is used instead.
  \lstinputlisting[linerange={40}]{scrolling-game/bin/usart.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={96,97}, caption={Absolute paths from the same crates.}]{scrolling-game/lib.rs}
\end{frame}

\begin{frame}
  \frametitle{Relative Paths}
  \lstinputlisting[linerange={110,111}, caption={Relative paths using \inlinecode{super}.}]{scrolling-game/lib.rs}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={15}, caption={Brining \inlinecode{pac} into scope.}]{scrolling-game/main.rs}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={32 -43}]{scrolling-game/game.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,311-314,315}]{scrolling-game/game.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,268-270,284,315}]{scrolling-game/game.rs}
\end{frame}
...
\begin{frame}
  ...
  \lstinputlisting[linerange={45,86-94,315}]{scrolling-game/game.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange={90}]{scrolling-game/main.rs}
\end{frame}
...
\begin{frame}[allowframebreaks]
  ...
  \lstinputlisting[linerange=50-52, caption={Functions taking ownership usage.}]{scrolling-game/bin/ownership.rs}

  \pagebreak
  \lstinputlisting[linerange=85-94, caption={Associated function gives ownership.}]{scrolling-game/game.rs}
\end{frame}
